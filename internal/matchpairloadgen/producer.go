...
// Producer generates random player pairs at high throughput and publishes MatchFound
// events to Kafka, simulating the output of the matchmaking service under load.
...
func (p *Producer) randomPair() (kafka.Message, error) {
	...
	id := uuid.New()
	payload, err := proto.Marshal(&matchmakingpb.MatchFound{
		MatchId:     id.String(),
...
// Run produces MatchFound events at the configured rate until ctx is cancelled.
// It spawns cfg.Workers goroutines, each ticking at rate/workers per second.
func (p *Producer) Run(ctx context.Context) {
	msgsPerWorker := max(p.cfg.Rate/p.cfg.Workers, 1)
	// Clamp to 1ms minimum (OS timer resolution floor).
	interval := max(time.Second/time.Duration(msgsPerWorker), time.Millisecond)

	slog.Info("match pair load generator started",
	...
	wg.Wait()
	slog.Info("match pair load generator stopped")
}
